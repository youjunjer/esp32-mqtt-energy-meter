...
void readSensors() {
  ...
  int raw = total / 10;
  lightPercent = constrain(map(raw, 0, 4095, 100, 0), 0, 100);
}
...
void drawInvalid(const char *status) {
  ...
  display.setCursor(0, 50); display.print("T="); display.print(temperature); display.print(" H="); display.print(humidity); display.print(" L="); display.print(lightPercent); display.print(" "); display.println(status);
  display.display();
}
...
void setup() {
  ...
  setRelay(true); // GPIO18 LOW by default.
