...
void setRelay(bool on) {
  ...
  Serial.print("Relay: ");
  Serial.println(on ? "ON (GPIO18 LOW)" : "OFF (GPIO18 HIGH)");
}
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (message.indexOf("\"RELAY\":\"ON\"") >= 0 ||
      message.indexOf("\"RELAY\": \"ON\"") >= 0) {
    setRelay(true);
  } else if (message.indexOf("\"RELAY\":\"OFF\"") >= 0 ||
             message.indexOf("\"RELAY\": \"OFF\"") >= 0) {
    setRelay(false);
  } else {
    Serial.print("Unknown relay message: ");
...
void drawInvalid(const char *status) {
  ...
  display.setCursor(0, 50); display.print("Hz=- PF=- "); display.print("R="); display.print(relayOn ? "ON " : "OFF "); display.println(status);
  display.display();
}
...
void drawEnergy(const EnergyData &data, const char *status) {
  ...
  display.setCursor(0, 50); display.print("Hz="); display.print(data.frequency, 1); display.print(" PF="); display.print(data.powerFactor, 2);
  display.print(" R="); display.print(relayOn ? "ON" : "OFF"); display.print(" "); display.println(status);
  display.display();
}
...
void setup() {
  ...
  setRelay(true); // Default startup: GPIO18 LOW, relay ON.
...
void loop() {
  // Keep MQTT callback processing continuously responsive.
